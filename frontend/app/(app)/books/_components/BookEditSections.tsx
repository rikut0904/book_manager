...
type BookEditSectionsProps = {
  bookId: string;
  book: Book | null;
  userBook: UserBook | null;
  seriesList: Series[];
  onRefreshUserBook: () => Promise<void>;
  initialSeriesId?: string;
  hasSeriesHint?: boolean;
  seriesSectionTitle?: string;
};

export default function BookEditSections({
  bookId,
  book,
  userBook,
  seriesList,
  ...
  initialSeriesId = "",
  hasSeriesHint = false,
  seriesSectionTitle = "シリーズ情報",
}: BookEditSectionsProps) {
  const [isSeries, setIsSeries] = useState(
    Boolean(userBook?.seriesId || initialSeriesId || hasSeriesHint)
  );
  const [seriesId, setSeriesId] = useState(
    userBook?.seriesId || initialSeriesId
  );
  const [volumeNumber, setVolumeNumber] = useState(
    userBook?.volumeNumber ? String(userBook.volumeNumber) : ""
  );
