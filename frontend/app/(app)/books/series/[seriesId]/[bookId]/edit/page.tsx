...
type SeriesBookEditSectionsProps = {
  bookId: string;
...
function SeriesBookEditSections({
  bookId,
  seriesIdParam,
  book,
  userBook,
  seriesList,
  onRefreshUserBook,
}: SeriesBookEditSectionsProps) {
  const [isSeries, setIsSeries] = useState(
    Boolean(userBook?.seriesId || seriesIdParam)
  );
  const [seriesId, setSeriesId] = useState(
    userBook?.seriesId || seriesIdParam
  );
...
export default function SeriesBookEditPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {bookId ? (
        <SeriesBookEditSections
          key={`${userBook?.id ?? "none"}-${seriesIdParam}`}
          bookId={bookId}
          seriesIdParam={seriesIdParam}
          book={book}
          userBook={userBook}
